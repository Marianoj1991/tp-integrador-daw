...
var Ripple = class _Ripple extends BaseComponent {
  ...
  onMouseDown(event) {
    ...
    !this.$unstyled() && M(ink, "p-ink-active");
    ink.setAttribute("data-p-ink-active", "false");
    if (!Tt(ink) && !Nt(ink)) {
      let d = Math.max(C(this.el.nativeElement), L(this.el.nativeElement));
      ink.style.height = d + "px";
      ink.style.width = d + "px";
    }
    let offset = _(this.el.nativeElement);
    let x = event.pageX - offset.left + this.document.body.scrollTop - Nt(ink) / 2;
    ...
    this.timeout = setTimeout(() => {
      ...
      if (ink2) {
        !this.$unstyled() && M(ink2, "p-ink-active");
  ...
  onAnimationEnd(event) {
    ...
    !this.$unstyled() && M(event.currentTarget, "p-ink-active");
...
//# sourceMappingURL=chunk-5T7FRX65.js.map
